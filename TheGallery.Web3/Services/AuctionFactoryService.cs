...
{
    public class AuctionFactoryService(HttpClient httpClient, IJSRuntime jsRuntime)
    {
        ...
        public async Task<string> GetContractOwnerAsync()
        {
            ...
            return await _web3.Eth.GetContract(_abi, _appConfig.ContractAddresses.AuctionFactory).GetFunction("owner").CallAsync<string>();
        }
        ...
        public async Task<string?> CreateAuctionAsync(Auction auction)
        {
            ...
            try
            {
                ...
                await _jsRuntime.InvokeAsync<string>("approveNFT", _appConfig.ContractAddresses.MockNFT, _abiNFT, _appConfig.ContractAddresses.AuctionFactory, auction.TokenId);
                var tx = await _jsRuntime.InvokeAsync<string>("newAuction", _appConfig.ContractAddresses.AuctionFactory, _abi, _appConfig.ContractAddresses.MockNFT, auction.TokenId, floorPriceWei.ToString(), reservePriceWei.ToString(), auction.Coefficient, auction.BuyNow, _appConfig.ContractAddresses.PaymentSplitter);
                return tx;
            }
            catch (Exception ex)
            {
